...
export default function FullExpandableAlertTable() {
  ...
  const [alertDetails, setAlertDetails] = useState<Record<number, AlertDetail | null>>({});
  ...
  const toggleRow = async (rowId: number, alertType: string) => {
    ...
    if (isOpen) {
      ...
      if (!alertDetails[rowId]) {
        ...
        setAlertDetails(prev => ({ ...prev, [rowId]: details as AlertDetail }));
        ...
        <TableBody>
          {initialAlerts.map(alert => (
            <React.Fragment key={alert.id}>
              <TableRow>
                <TableCell>
                  <IconButton size="small" onClick={() => toggleRow(alert.id, alert.type)}>
                    {expandedRows.includes(alert.id) ? <ArrowUpIcon /> : <ArrowDownIcon />}
                  </IconButton>
                </TableCell>
                <TableCell>{alert.type}</TableCell>
                <TableCell>
              ...
              <TableRow>
                <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={3}>
                  <Collapse in={expandedRows.includes(alert.id)} timeout="auto" unmountOnExit>
                    <Box margin={2}>
                      {loadingRows.includes(alert.id) ? (
                        ...
                        <>
                          <Typography variant="body2" gutterBottom>
                            <strong>Info:</strong> {alertDetails[alert.id]?.info}
                          </Typography>
                          <Typography variant="body2" gutterBottom>
                            <strong>Recommendation:</strong> {alertDetails[alert.id]?.recommendation}
                          </Typography>
                          {!resolvedRows.includes(alert.id) && (
                            <Button
                              variant="contained"
