...
class PermissionTests(TestCase):
    ...
    def test_is_professor_true(self):
        user = get_user_model().objects.create_user(email='prof@example.com', password='testpass')
        user.groups.add(self.professor_group)
        self.assertTrue(is_professor(user))

    def test_is_professor_false(self):
        user = get_user_model().objects.create_user(email='regular@example.com', password='testpass')
        self.assertFalse(is_professor(user))

    def test_is_ta_true(self):
        user = get_user_model().objects.create_user(email='ta@example.com', password='testpass')
    ...
    def test_is_admin_superuser(self):
        user = get_user_model().objects.create_superuser(email='super@example.com', password='testpass')
        self.assertTrue(is_admin(user))
    ...
    def test_is_professor_or_ta_none(self):
        ...
        permission = IsProfessorOrTA()
        request = self.factory.get('/')
        user = get_user_model().objects.create_user(email='none@example.com', password='testpass')
        request.user = user
